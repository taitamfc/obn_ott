...
class SettingController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('auth_page_background_image')) {
            $imagePath = $this->uploadFile($request->file('auth_page_background_image'), $this->user_id.'/banners/images');
            Setting::where('setting_name', 'auth_page_background_image')->where('user_id',$this->user_id)->update([ 'setting_value' => $imagePath ]);
        }
        if ($request->hasFile('plan_page_background_image')) {
            $imagePath = $this->uploadFile($request->file('plan_page_background_image'), $this->user_id.'/banners/images');
            Setting::where('setting_name', 'plan_page_background_image')->where('user_id',$this->user_id)->update([ 'setting_value' => $imagePath ]);
        }

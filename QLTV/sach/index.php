...
$sql_count = "SELECT COUNT(*) AS total_records FROM sach";
...
if (isset($_GET["s"]) && !empty(trim($_GET["s"]))) {
    ...
    $sql .= " WHERE sach.tensach LIKE '%$s%' OR sach.tentacgia LIKE '%$s%' OR sach.ngayxuatban LIKE '%$s%' OR loaisach.tenloaisach LIKE '%$s%'" ;
}
else{
...
<div class="container-fluid px-4">
    <a class="btn btn-success mb-3" href="add.php"> Thêm mới </a><br>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            <?php foreach ($sach as $key => $row) : ?>
                <tr>
                    <td><?php echo $start_index + $key + 1; ?></td>
